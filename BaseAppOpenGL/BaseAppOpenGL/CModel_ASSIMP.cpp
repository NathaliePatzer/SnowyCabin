...
CModel_ASSIMP::CModel_ASSIMP(RENDER_MODE render, std::string modelPathAndName)
{
	...
	else if (this->iRenderMode == RENDER_MODE::VERTEX_ARRAY)
	{
		...
		{
			meshEntries.push_back(new CModel_ASSIMP::MeshEntry(scene->mMeshes[i], false));
		}
	}
	else if (this->iRenderMode == RENDER_MODE::VERTEX_BUFFER_OBJECT_MODE)
	{
		for (int i = 0; i < scene->mNumMeshes; ++i)
		{
			meshEntries.push_back(new CModel_ASSIMP::MeshEntry(scene->mMeshes[i], true));
		}
	}
	else {}
...
CModel_ASSIMP::~CModel_ASSIMP()
{
	...
	if (this->iRenderMode == RENDER_MODE::VERTEX_BUFFER_OBJECT_MODE)
	{
		for (int i = 0; i < meshEntries.size(); ++i)
		{
			delete meshEntries.at(i);
		...
		for (t = 0; t < mesh->mNumFaces; ++t)
		{
			...
			for (i = 0; i < face->mNumIndices; i++)		// go through all vertices in face
			{
				...
				if (mesh->HasTextureCoords(0))		//HasTextureCoords(texture_coordinates_set)
				{
					if (this->bump == true)
					{
						glMultiTexCoord2f(GL_TEXTURE0, mesh->mTextureCoords[0][vertexIndex].x, 1 - mesh->mTextureCoords[0][vertexIndex].y);
						glMultiTexCoord2f(GL_TEXTURE1, mesh->mTextureCoords[0][vertexIndex].x, 1 - mesh->mTextureCoords[0][vertexIndex].y);
					}
					else
...
CModel_ASSIMP::MeshEntry::MeshEntry(aiMesh* mesh, bool isVBO)
{
	glGenBuffers(4, BufferNameVBO);
	...
	if (mesh->HasPositions())
	{
		...
		for (int i = 0; i < mesh->mNumVertices; ++i)
		{
			...
			glm::vec3 v = glm::vec3(mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z);
			vVertexData.push_back(v);
		}
	}
	...
	if (mesh->HasTextureCoords(0))
	{
		...
		for (int i = 0; i < mesh->mNumVertices; ++i)
		{
			...
			glm::vec2 t = glm::vec2(mesh->mTextureCoords[0][i].x, (1 - mesh->mTextureCoords[0][i].y));
			vTexCoordData.push_back(t);
		}
	}
...
CModel_ASSIMP::MeshEntry::~MeshEntry()
{
	...
	glDeleteBuffers(4, BufferNameVBO);
}
...
void CModel_ASSIMP::MeshEntry::CreateVBO()
{
	glBindBuffer(GL_ARRAY_BUFFER, BufferNameVBO[V_DATA]);
	glBufferData(GL_ARRAY_BUFFER, this->vVertexData.size() * sizeof(glm::vec3), &this->vVertexData.front(), GL_STATIC_DRAW);
	glVertexPointer(3, GL_FLOAT, 0, 0);

	glBindBuffer(GL_ARRAY_BUFFER, BufferNameVBO[N_DATA]);
	glBufferData(GL_ARRAY_BUFFER, this->vNormalData.size() * sizeof(glm::vec3), &this->vNormalData.front(), GL_STATIC_DRAW);
	glNormalPointer(GL_FLOAT, 0, 0);

	glBindBuffer(GL_ARRAY_BUFFER, BufferNameVBO[T_DATA]);
	glBufferData(GL_ARRAY_BUFFER, this->vTexCoordData.size() * sizeof(glm::vec2), &this->vTexCoordData.front(), GL_STATIC_DRAW);
	glTexCoordPointer(2, GL_FLOAT, 0, 0);


}

void CModel_ASSIMP::MeshEntry::renderVBO()
{
	glEnableClientState(GL_VERTEX_ARRAY);
	glBindBuffer(GL_ARRAY_BUFFER, BufferNameVBO[V_DATA]);
	glVertexPointer(3, GL_FLOAT, 0, 0);
